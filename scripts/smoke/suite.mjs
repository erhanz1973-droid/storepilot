...
function resolveMetaEncryptionSecret() {
    ...
    if (shopify && shopify.length >= 32)
        return shopify;
    return null;
}
function resolveGoogleEncryptionSecret() {
    const google = process.env.GOOGLE_ADS_TOKEN_ENCRYPTION_KEY?.trim();
    if (google && google.length >= 32)
        return google;
...
async function checkShopify() {
    return timed("Shopify", async () => {
        ...
        let query = supabase
            ...
            .select("store_id, shop_domain, access_token_encrypted, status, connection_health, client_id, sync_stats, scopes")
            .eq("status", "active")
            .order("installed_at", { ascending: false });
        if (shopFilter)
            query = query.eq("shop_domain", shopFilter);
        const { data: rows, error } = await query.limit(1);
        if (error)
            throw new Error(`Installation query failed: ${error.message}`);
        const row = rows?.[0];
        if (!row) {
        ...
        const accessToken = decryptAesGcm(row.access_token_encrypted, resolveShopifyEncryptionSecret());
        ...
        let productsResult;
        try {
            productsResult = await shopifyGraphQL(shop, accessToken, `query { products(first: 5) { edges { node { id title } } } }`);
        }
        catch (error) {
            return {
                status: "FAIL",
        ...
        const countRes = await fetch(`https://${shop}/admin/api/${SHOPIFY_API_VERSION}/products/count.json`, {
        ...
        const ordersResult = await shopifyGraphQL(shop, accessToken, `query { orders(first: 1, sortKey: CREATED_AT, reverse: true) { edges { node { id } } } }`);
        ...
        const customersResult = await shopifyGraphQL(shop, accessToken, `query { customers(first: 1) { edges { node { id } } } customersCount { count } }`);
        ...
        const inventoryResult = await shopifyGraphQL(shop, accessToken, `query {
        products(first: 5) {
        ...
        const discountsResult = await shopifyGraphQL(shop, accessToken, `query { discountNodes(first: 5) { edges { node { id } } } }`);
        ...
        if (productsCount <= 0) {
            return {
                ...
                details: { shop, appMatch, status: row.status },
            };
        }
        const syncStats = (row.sync_stats ?? {});
        return {
            status: "PASS",
            message: discountStatus === "unavailable"
...
async function checkMetaAds(storeId) {
    return timed("Meta Ads", async () => {
        try {
            const supabase = getSupabase();
            let query = supabase
                .from("meta_ads_installations")
                .select("id, store_id, status, connection_health, ad_account_id, access_token_encrypted, sync_stats")
                .eq("status", "active");
            if (storeId)
            ...
            let token;
            try {
                token = decryptAesGcm(row.access_token_encrypted, secret);
            }
            catch {
                return {
                    status: "WARNING",
            ...
            const meRes = await fetch(`https://graph.facebook.com/v21.0/me?fields=id,name&access_token=${encodeURIComponent(token)}`, { cache: "no-store" });
            if (!meRes.ok) {
                return {
                    status: "WARNING",
            ...
            const campaignsRes = await fetch(`https://graph.facebook.com/v21.0/act_${String(accountId).replace(/^act_/, "")}/campaigns?fields=id,name,status&limit=5&access_token=${encodeURIComponent(token)}`, { cache: "no-store" });
            if (!campaignsRes.ok) {
                return {
                    status: "WARNING",
...
async function checkGoogleAds(storeId) {
    return timed("Google Ads", async () => {
        try {
            const supabase = getSupabase();
            let query = supabase
                .from("google_ads_installations")
                .select("id, store_id, status, connection_health, customer_id, refresh_token_encrypted, access_token_encrypted, sync_stats")
                .eq("status", "active");
            if (storeId)
                query = query.eq("store_id", storeId);
            const { data: rows, error } = await query.limit(1);
            ...
            const refreshEncrypted = row.refresh_token_encrypted ||
            ...
            const searchRes = await fetch(`https://googleads.googleapis.com/${apiVersion}/customers/${customerId}/googleAds:search`, {
                method: "POST",
                headers: {
...
async function checkGa4(storeId) {
    return timed("GA4", async () => {
        try {
            const supabase = getSupabase();
            let query = supabase
                .from("ga4_installations")
                .select("id, store_id, status, connection_health, property_id, property_name, access_token_encrypted, refresh_token_encrypted, last_sync_at")
                .eq("status", "active");
            if (storeId)
                query = query.eq("store_id", storeId);
            const { data: rows, error } = await query.limit(1);
            ...
            const clientSecret = process.env.GA4_CLIENT_SECRET?.trim() || process.env.GOOGLE_ADS_CLIENT_SECRET?.trim();
...
async function httpGetHtml(baseUrl, path) {
    const response = await fetch(`${baseUrl.replace(/\/$/, "")}${path}`, {
        method: "GET",
...
function hasReactServerError(html) {
    return (/Application error: a (client|server)-side exception/i.test(html) ||
...
export async function runDirectSmokeChecks(options) {
    ...
    checks.push(shopify);
    const storeId = shopify.status === "PASS" && typeof shopify.details?.storeId === "string"
    ...
    checks.push(await timed("Executive Dashboard", async () => {
    ...
    checks.push(await timed("Executive Digest", async () => {
        ...
        const hasSummary = Boolean(body.storeMetrics) ||
    ...
    checks.push(await timed("Recommendation Engine", async () => {
        ...
        const recommendations = res.json.recommendations ?? [];
    ...
    checks.push(await timed("Autopilot", async () => {
    ...
    checks.push(await timed("Simulation Lab", async () => {
        ...
        return {
            ...
            details: { keys: Object.keys(result).slice(0, 12) },
        };
    }));
    return checks;
}
export function finalizeSmokeReport(input) {
    ...
    return {
        ...
        deploymentId: process.env.RAILWAY_DEPLOYMENT_ID?.trim() ||
            ...
            null,
        commitHash: process.env.RAILWAY_GIT_COMMIT_SHA?.trim() ||
