...
export function rawFactsFromSnapshot(
  ...
  return {
    ...
    window: "last30d",
    commerce: {
      revenue: rollups?.revenue ?? snapshot.storeMetrics.revenue30d ?? 0,
      orders: rollups?.orders ?? snapshot.storeMetrics.orders30d ?? 0,
      ...
      platformFees: 0,
      sessions: snapshot.ga4Snapshot?.sessions30d ?? null,
      customers: rollups?.orders ?? snapshot.storeMetrics.orders30d ?? 0,
      inventoryUnits: null,
      inventoryValue: null,
    },
    advertising: {
